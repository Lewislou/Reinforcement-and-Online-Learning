...
def bp_train(feature, label, n_hidden, maxCycle, alpha, n_output,center, delta, w):
    '''计算隐含层的输入
    ...
    while iter <= maxCycle:
        ...
        # 2.1.1、计算隐含层的输出
        index = 0
        hidden_output = hidden_out(feature[index,:],center,delta)
        # 2.1.3、计算输出层的输入
        output_in = predict_in(hidden_output, w)  
        #print('out_put_in',output_in.shape)
        # 2.1.4、计算输出层的输出
        output_out = predict_out(output_in)
        # 2.2、误差的反向传播
        error = mat(label[index] - output_out).reshape(3,1)
        ...
        for j in range(n_hidden):
            ...
            sum2 = 0.0
            sum3 = 0.0
            sum1 = error * exp(-1.0 * (feature[index]-center[j]) * (feature[index]-center[j]).T / (2 * delta[0,j]*delta[0,j])) * (feature[index] - center[j])
            sum2 = error * exp(-1.0 * (feature[index]-center[j]) * (feature[index]-center[j]).T / (2 * delta[0,j]*delta[0,j])) * (feature[index] - center[j]) * (feature[index] - center[j]).T
            sum3 = error * exp(-1.0 * (feature[index]-center[j]) * (feature[index]-center[j]).T / (2 * delta[0,j]*delta[0,j]))
       ...
       # 2.3、 修正权重和rbf函数中心和扩展常数       
            ...
            w[j,:] = w[j,:] + alpha * delta_w.reshape(1,3)
